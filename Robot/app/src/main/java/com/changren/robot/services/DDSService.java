...
public class DDSService extends Service {
    ...
    private static final String TAG = DDSService.class.getSimpleName();

    private int mAuthCount = 0;// 授权次数,用来记录自动授权

    public static Intent getLaunchIntent(Context context){
        return new Intent(context, DDSService.class);
    }
    ...
    public void checkDDSReady(){
        while (true){
            XLog.e(TAG, "【checkDDSReady】InitStatus:" + DDS.getInstance().getInitStatus());
...
//        config.addConfig(DDSConfig.K_API_KEY, "6463eb2fb043af6add7c48aa5c85f4c7");
        //填入唯一的deviceId -- 选填
        config.addConfig(DDSConfig.K_DEVICE_ID, getDeviceId(getApplicationContext()));
//        config.addConfig(DDSConfig.K_DEVICE_ID, "");
    ...
    private DDSInitListener mInitListener = new DDSInitListener() {
        ...
        public void onInitComplete(boolean isFull) {
            ...
            if (isFull){
                //发送一个init成功的广播
                LocalBroadcastManager.getInstance(DDSService.this).sendBroadcast(new Intent(RobotConstant.ACTION_INIT_COMPLETE));
            }
        }
    ...
    private DDSAuthListener mAuthListener = new DDSAuthListener() {
        ...
        public void onAuthSuccess() {
            ...
            startService(DUIService.getLaunchIntent(DDSService.this));
        }
